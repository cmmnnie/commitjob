    ...
    try {
      ...
      if (mcpResponse.data && mcpResponse.data.success && mcpResponse.data.recommendations) {
        ...
        for (const job of rerankedJobs) {
          ...
          // 빅데이터/AI 우선 배정 (10개 제한)
          if (isDataAi && dataAiJobs.length < 10) {
            dataAiJobs.push(job);
            usedJobIds.add(job.job_id);
          }
          // IT만 해당되고 빅데이터/AI에 배정되지 않은 경우만 (10개 제한)
          else if (isIt && !usedJobIds.has(job.job_id) && itJobs.length < 10) {
            itJobs.push(job);
            usedJobIds.add(job.job_id);
          }

          // 두 카테고리 모두 10개씩 채워지면 종료
          if (dataAiJobs.length >= 10 && itJobs.length >= 10) break;
        }
