...
-- 사용자 면접 조건 입력 이력 테이블 (추가)
CREATE TABLE IF NOT EXISTS user_interview_conditions (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT,
    session_id VARCHAR(100),
    preferred_job VARCHAR(200), -- 희망 직무
    company_size VARCHAR(50), -- 회사 규모 선호도
    industry VARCHAR(200), -- 관심 산업/분야
    additional_skills JSON, -- 추가 기술/스킬 ["AWS", "Docker", "Kubernetes"]
    custom_company VARCHAR(200), -- 직접 입력한 회사명 (있는 경우)
    custom_position VARCHAR(200), -- 직접 입력한 포지션 (있는 경우)
    input_source VARCHAR(50) DEFAULT 'interview_form', -- 'interview_form', 'company_selection', 'custom_input' 등
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL,
    INDEX idx_user_id (user_id),
