"""
catch.db의 회사 정보를 기반으로 MySQL에 회사 정보와 리뷰 데이터 생성
기존 companies 테이블 구조를 사용하고 새로운 catch_companies/catch_reviews 테이블 생성
"""
...
RATINGS = ['4.5', '4.0', '3.5', '5.0', '4.2', '3.8', '4.7', '4.3', '3.9', '4.6']

def create_tables():
    """MySQL에 catch_companies와 catch_reviews 테이블 생성"""
    conn = mysql.connector.connect(**MYSQL_CONFIG)
    cursor = conn.cursor()

    # catch_companies 테이블 생성 (캐치 사이트에서 수집한 회사 정보)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS catch_companies (
    ...
    # catch_reviews 테이블 생성
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS catch_reviews (
            id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    cursor.close()
    conn.close()
    print("✅ 테이블 생성 완료 (catch_companies, catch_reviews)")
...
def generate_company_data(company_name):
    ...
    return {
        ...
        'ceo': f'{company_name[:2]}대표' if len(company_name) >= 2 else f'{company_name}대표',
    ...
    try:
        cursor.execute("""
            INSERT INTO catch_companies (
                name, industry, company_type, location, employee_count,
        ...
        cursor.execute("SELECT id FROM catch_companies WHERE name = %s", (company_data['name'],))
...
def generate_reviews(num_reviews=10):
    ...
    for _ in range(num_reviews):
        reviews.append({
            ...
            'rating': float(random.choice(RATINGS)),
...
def main():
    ...
    print("\n" + "=" * 60)
    print("데이터 생성 완료!")
    print(f"✅ 총 {total_companies}개 회사 정보 저장 (catch_companies 테이블)")
    print(f"✅ 총 {total_reviews}개 리뷰 저장 (catch_reviews 테이블)")
    print("=" * 60)
