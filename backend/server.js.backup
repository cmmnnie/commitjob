...
app.post('/api/scrape-latest-jobs', async (req, res) => {
  ...
  try {
    ...
    if (scrapedJobs.length === 0) {
      ...
      return res.status(500).json({
        success: false,
        message: 'Catch 사이트에서 공고를 가져오지 못했습니다. IT 필터 또는 빅데이터·AI 필터 적용이 실패했을 수 있습니다.'
      });
    }
